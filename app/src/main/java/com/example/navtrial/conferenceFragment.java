...
public class conferenceFragment extends Fragment {
    private RecyclerView conferenceadsRecyclerView;
    ProgressDialog progressDialog;
    private RecyclerView.Adapter conferenceadsRecyclerAdapter;
    ArrayList<event> conferenceads;
    View view;
    @Nullable
    @Override
    ...
    public void getConferenceAds(List<event> allads){
        conferenceads = new ArrayList<>();
        for(int i = 0;i<allads.size();i++){
            if(allads.get(i).getCategory().equals("conference")){
                conferenceads.add(allads.get(i));

            }
        }
        conferenceadsRecyclerView = view.findViewById(R.id.conference_recycler_view);
        conferenceadsRecyclerAdapter = new conferenceAdapter(getContext(),conferenceads);
        conferenceadsRecyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        conferenceadsRecyclerView.setAdapter(conferenceadsRecyclerAdapter);
