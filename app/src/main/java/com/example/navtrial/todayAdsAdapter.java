...
public class todayAdsAdapter  extends RecyclerView.Adapter<todayAdsAdapter.ViewHolder> {
    private List<event> ads;
    private Context context;
    private List<event> mevents;

    public todayAdsAdapter(Context context,List<event> ads){
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.today_ads_recyclerview,parent,false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if(ads!=null){
          ...
          //Picasso.get().load("http://10.0.2.2:3000/api/containers/ads/download/" + ad.getImage()).into(holder.im);
             Picasso.get().load("http://10.5.231.160:3000/api/containers/ads/download/" + ad.getImage()).into(holder.im);
